  ...
  $(function(){
    ...
    $(document).on('click','.update',function(){
      ...
      $.post('data_after.php', {
        ...
        // $('#overlay_day, #modalWindow_input').fadeOut();
        $('#'+ works).empty();
      ...
      $.post('select_month.php',{
        ...
        for(n in result){
          if(result[n].obj == ""){
            ...
            //時間も取得する場合
            // $('#select_month_view_tb').append('<tr><td class="day" width="60px">' + result[n].day.slice(5,10) + '</td><td>' + result[n].str + '</td><td>' + result[n].end + '</td><td>'+ result[n].obj + '</td></tr>');
            $('#select_month_view_tb').append('<tr><td class="day" width="60px">' + result[n].day.slice(5,10) + '</td><td>' + result[n].obj + '</td></tr>');
          }
        }
      });
